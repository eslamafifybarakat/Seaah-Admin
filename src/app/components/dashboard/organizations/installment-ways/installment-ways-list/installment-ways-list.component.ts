...
export class InstallmentWaysListComponent {
  ...
  private updateMetaTagsForSEO(): void {
    let metaData: MetaDetails = {
      title: 'طرق التقسيط | سعة',
      description: 'طرق التقسيط | سعة',
  ...
  getAllInstallmentWays(isFiltering?: boolean): void {
    ...
    let insallmentWaysSubscription: Subscription = this.installmentWaysService?.getInstallmentWaysList(this.page, this.perPage, this.searchKeyword, this.sortObj, this.filtersArray ?? null)
      .pipe(
        tap((res: any) => this.processInstallmentWaysListResponse(res)),
  ...
  // Installment Ways Details
  itemDetails(item?: any): void {
    this.router.navigate(['/Dashboard/Organizations/InstallmentWays/Details/' + item?.id]);
  }
  // Add Edit Installment Ways
  ...
  deleteItem(item: any): void {
    ...
    let deleteBankSubscription: Subscription = this.installmentWaysService?.deleteInstallmentWayById(item?.item?.id)?.pipe(
  ...
  //End Delete Installment Ways Functions
  ...
  searchHandler(keyWord: any): void {
    ...
    this.publicService?.changePageSub?.next({ page: this.page });
    this.installmentWaysList?.length <= 0 && this.dataStyleType == 'list' ? this.getAllInstallmentWays(true) : '';
    this.dataStyleType == 'grid' ? this.getAllInstallmentWays() : '';
    if (keyWord?.length > 0) {
